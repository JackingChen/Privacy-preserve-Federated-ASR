...
class Model(SingleForwardModel):
    ...
    # You can override methods from the parent class if needed
    def _get_embedding(self,inp,inp_embed_type, inp_model, pooler):
        if inp_embed_type == "mbert":
            out = inp_model(inp)[1]
        elif inp_embed_type == "xlm":
            out = inp_model(inp)[0]
            out = pooler(out)
    ...
    def forward(self, inp1, inp2):
        ...
        out2 = self._get_embedding(inp1,self.inp2_embed_type, self.inp2_model, self.pooler2)
    ...
    def preprocess_dataframe(self):
        ...
        self.df_train=self._Tokenize(df_train, self.inp1_embed_type, self.inp1Arg.inp_col_name)
        self.df_dev=self._Tokenize(df_dev, self.inp1_embed_type, self.inp1Arg.inp_col_name)
        self.df_test=self._Tokenize(df_test, self.inp1_embed_type, self.inp1Arg.inp_col_name)

        self.df_train=self._Tokenize(self.df_train, self.inp2_embed_type,self.inp2Arg.inp_col_name)
        self.df_dev=self._Tokenize(self.df_dev, self.inp2_embed_type,self.inp2Arg.inp_col_name)
        self.df_test=self._Tokenize(self.df_test, self.inp2_embed_type,self.inp2Arg.inp_col_name)
    ...
    def _df2Dataset(self):
        def DecideDtype(inp_embed_type):
            if inp_embed_type in Text_pretrain:
                ...
                dtype=torch.float
            return dtype
        dtype1=DecideDtype(self.inp1_embed_type)
        dtype2=DecideDtype(self.inp2_embed_type)
        ...
        self.val_data = TensorDataset(
             torch.tensor(self.df_val[self.inp1Arg.inp_col_name].tolist(), dtype=dtype1),
             torch.tensor(self.df_val[self.inp2Arg.inp_col_name].tolist(), dtype=dtype2),
    ...
    def _save_test_results_to_csv(self, df_result, pred_dict, args, suffix):
