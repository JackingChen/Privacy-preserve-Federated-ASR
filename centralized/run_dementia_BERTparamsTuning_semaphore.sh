...
if [ "$stage" -le 1 ]; then
    ...
    test_mdls=(mbert_sentence emilyalsentzer/Bio_ClinicalBERT)
    lrs=(2e-5 1e-5 1e-6 5e-7 1e-7)
    # lrs=(1e-1)
    # patiences=(3 4 5 6 7 8 9)
    patiences=(3 4)
    # lr_schedulers=(exp lin cos)
            ...
            for patience in "${patiences[@]}"; do
                for lr_scheduler in "${lr_schedulers[@]}"; do
                    for epoch in "${epochs[@]}";do
                        ...
                        #         --patience $patience &
                        cmd="python 0207_DM_SentenceLvl1input_ParamTuning.py --inp_embed $inpm --epochs $epoch --lr $lr --lr_scheduler $lr_scheduler --patience $patience"
